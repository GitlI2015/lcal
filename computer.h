...
#define GM 0.5772156649
#define EL 2.7182818284
            ...
            if (stkl[k] < m)
            {
                if (lcount == rcount)
                {
                    if (k + 1 == BRKT_MAX || stkl[k + 1] == 0)
                        return (stkr[j] - stkl[lb_pos] - 1);//·µ»Ø×óÓÒÁ½À¨ºÅÖ®¼äµÄ³¤¶È
                    if (stkl[k + 1] < stkr[j])
                    {
                        lcount = 1;
                        break;
...
double StringToFloat(char * args, int * start)
{
    ...
    if (args[k] == '.')
    {
        printf("wrong form of float number");
        err_sgn = 1;
        return 0;
    }
    *start = k - 1;
...
double MacroReplace(char * args, int * start)
{
    ...
    if (args[(*start) + 1] == '[')
    {
        ...
        do
        {
            ...
            if ((*start) == length)
            {
                printf("square brackets don't fit\n");
                err_sgn = 1;
                return 0;
            }
            (*start)++;
        ...
        {
            printf("I can't identify the function \"");
    ...
    else
    {
        ...
        else
        {
            ...
            printf("I can't identify the macro \"");
            for (i = temp; i <= (*start); i++)
                printf("%c", args[i]);
            printf("\"\n");
            err_sgn = 1;
